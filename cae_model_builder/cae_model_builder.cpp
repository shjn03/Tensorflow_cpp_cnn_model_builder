...
int main() {
	...
	auto input = ops::Placeholder(scope.WithOpName("input"), DT_FLOAT, ops::Placeholder::Shape({ num_batchs, image_width,image_height,image_channels }));
	...
	Variable W3(scope, { encode_width*encode_height*num_filters2, intermediate_dim }, DT_FLOAT);
	...
	Variable W5(scope, { encode_dim, encode_width*encode_height*num_filters2}, DT_FLOAT);
	Variable B5(scope, { encode_width*encode_height*num_filters2 }, DT_FLOAT);
	auto x5_1 = ops::BatchMatMul(scope.WithOpName("fc3"), x4_3, W3);
	auto x5_2 = ops::Add(scope.WithOpName("add5"), x5_1, B3);
	...
	auto x6_1 = ops::Reshape(scope.WithOpName("rehape2"), x5_3, { num_batchs,  encode_width,encode_height,num_filters2 });
	//Upsamplingの代替
	auto x6_2 = ops::ResizeNearestNeighbor(scope.WithOpName("maxunpool1"), x6_1, { encode_width * 2,encode_height * 2 });
	...
	//Upsamplingの代替
	auto x7_1 = ops::ResizeNearestNeighbor(scope.WithOpName("maxunpool2"), x6_5, { image_width,image_height });
	...
	auto output = ops::Placeholder(scope.WithOpName("output"), DT_FLOAT, ops::Placeholder::Shape({ num_batchs,image_width,image_height,image_channels }));
	...
	auto assign_w3 = Assign(scope, W3, RandomNormal(scope, { encode_width*encode_height*num_filters2, intermediate_dim }, DT_FLOAT));
	auto assign_w4 = Assign(scope, W4, RandomNormal(scope, { intermediate_dim,encode_dim}, DT_FLOAT));
	auto assign_w5 = Assign(scope, W4, RandomNormal(scope, { encode_dim, encode_width*encode_height*num_filters2 }, DT_FLOAT));
	...
	auto assign_b5 = Assign(scope, B5, RandomNormal(scope, { encode_width*encode_height*num_filters2 }, DT_FLOAT));
	...
	Tensor x_batch(DT_FLOAT, TensorShape({ num_batchs, image_width,image_height,image_channels }));
	Tensor y_batch(DT_FLOAT, TensorShape({ num_batchs, image_width,image_height,image_channels }));

	//test data
	Tensor x_test(DT_FLOAT, TensorShape({ num_tests,image_width,image_height,image_channels }));
	Tensor y_test(DT_FLOAT, TensorShape({ num_tests, image_width,image_height,image_channels }));
